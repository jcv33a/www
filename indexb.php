...
$inicio = ($pagina - 1) * $TAMANO_PAGINA; 
  ...
  // Retrieve the score data from MySQL
	...
	$query = "SELECT * FROM guitarwars where categoria='$categoria' "; 
	$data = mysqli_query($dbc, $query);	
	$num_total_registros = mysqli_num_rows($data); 
	//calculo el total de páginas 
	$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
	
	$query = "SELECT * FROM guitarwars WHERE categoria='$categoria' " . " ORDER BY date DESC limit " . $inicio . "," . $TAMANO_PAGINA;
    $data = mysqli_query($dbc, $query);
...
if (is_file(GW_UPLOADPATH . $row['screenshot']) && filesize(GW_UPLOADPATH . $row['screenshot']) > 0) {
	  ...
	  echo '<td><li><a target="_blank" href="images/'.$row['screenshot'].'"><img src="' . GW_UPLOADPATH . $row['screenshot'] . '" width="400px" height="350px" alt="Score image" /></a></li></td></tr>';
...
if (is_file(GW_UPLOADPATH . $row['screenshot2']) && filesize(GW_UPLOADPATH . $row['screenshot2']) > 0) {
	  ...
	  echo '<td><li><a target="_blank" href="images/'.$row['screenshot2'].'"><img src="' . GW_UPLOADPATH . $row['screenshot2'] . '" width="400px" height="300px" alt="Score image" /></a></li></td>';	  
...
if (is_file(GW_UPLOADPATH . $row['screenshot3']) && filesize(GW_UPLOADPATH . $row['screenshot3']) > 0) {
	  ...
	  echo '<td><li><a target="_blank" href="images/'.$row['screenshot3'].'"><img src="' . GW_UPLOADPATH . $row['screenshot3'] . '" width="400px" height="300px" alt="Score image" /></a></li></td></tr>';	  
...
$inicio = ($pagina - 1) * $TAMANO_PAGINA; 
  ...
  // Retrieve the score data from MySQL
	...
	$query = "select * from guitarwars " . " ORDER BY date DESC limit " . $inicio . "," . $TAMANO_PAGINA;
	$data = mysqli_query($dbc, $query);
