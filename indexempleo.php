...
$inicio = ($pagina - 1) * $TAMANO_PAGINA; 
  ...
  // Retrieve the score data from MySQL
	...
	$query = "select * from guitarwars where categoria='empleo'";
	$data = mysqli_query($dbc, $query);	
	$num_total_registros = mysqli_num_rows($data); 
	//calculo el total de páginas 
	$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);  

	$query = "select * from guitarwars where categoria='empleo'" . " ORDER BY date DESC limit " . $inicio . "," . $TAMANO_PAGINA;
	$data = mysqli_query($dbc, $query);
...
echo "<a href='indexempleo.php?pagina=" . $i . "'>" . $i . "</a> "; 
} 
} 
